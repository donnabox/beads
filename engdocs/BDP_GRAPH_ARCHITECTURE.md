...
**Status:** Draft v8 (W-arch, after six council rounds; the fence cell and the counter nonce are probe-confirmed on Dolt 2.1.8) — feat/bead-graph
**Date:** 2026-09-02
...
| A9 | **9** — *new, optional simplification* | — | **v0 authority requires a shared database.** If ruled: hazard R's publication primitive, remote-read freshness, and multi-phase publication recovery are **deferred to the write-profile ADR**; a remote-backed workspace is a **non-authority** for any Scope it did not mint on its own shared database (its `bd serve` shows rows absent; its CLI reads refuse); cross-database promotion is **`--rotate-url` only** (a new Scope, continuity via the ledger lane). The graph tables still replicate through push/pull like any other table; the validator refuses foreign deltas. What remains of A7 is hazard S alone — one arbiter, one lease, one counter. **Cut sheet, normative if ruled:** only a shared database can mint, promote, rotate, install, write, read, or serve; the embedded and registered-store arms refuse every authority-establishing and authority-dependent graph operation (`Mint`/`Promote`/`Rotate`/`Install` and CLI graph reads answer `ErrNotAuthority: authority requires a shared database`; the embedded leg exists only as the `bdp.client: server` host); a configured remote neither grants nor removes authority, and the lease row `Mint` created is the sole proof of "minted here"; every hazard-R passage (publication primitive, remote-read freshness, multi-phase publication recovery, `Remote` fixture cases, Part D.5) is excluded; A5's "single-copy by operator responsibility" sentence is withdrawn. **Recommended for v0**: the only serving topology already is the SQL-server workspace, and this removes the largest block of new mechanism from P1/P2. | Round 5 (Codex C1: "require an externally observable fence or weaken the guarantee"). |
...
internal/storage/graphops/       TX-LEVEL SHARED BODY — all three legs call it
  ...
  assertAuthorityInTx(ctx, tx, w, claim, mutating): Scope row == w; ledger head; lease row —
    read: SELECT holder/epoch/fence (no expiry term); mutation: UPDATE … SET heartbeat_at = ?,
...
- **`graphops.ScopeBootstrapper`** — `Mint`, once, under the exclusive gate
  through a temporary source (staged startup: `bd bdp serve` bypasses the
  generic pre-run store gate, reads the Scope row under a temporary source
  and the shared gate, and takes the exclusive gate only when there is no
  Scope row, re-checking under it), fenced per hazard, multi-phase: precondition *no Scope row*; INSERT the singleton Scope row, seed
...
- **`graphops.Admin`** — `Promote` (precondition: Scope row CAS + the
  ...
  composition root — `Promote`, `Rotate`, and `Install` under the **shared**
  gate (they rely on the lease and run beside a live server), `LedgerApply`
...
BeadGraphReader (manager-backed role): w := authority.Load(beadsDir); claim := process claim
   ...
   │ stmt 3  lease row: holder key/epoch == claim   (SELECT — a read; a mutation UPDATEs it,
   │         rewriting the fence cell and extending expires_at, and self-regrants if expired)
