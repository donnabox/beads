...
tree's own conventions were read closely. Five three-reviewer councils
(Claude, Codex, Gemini) reviewed v1–v5; §2 records what changed and which
...
invented primitive with one the tree already has. v6 also offers the
operator a **simplification ruling (A9)** that deletes most of what remains.
...
restore, a copied file, or a promotion elsewhere is refused by the store,
and no caller can supply the witness. A shared database is fenced by that
...
| `DOLT_RESET --hard` to undo a refused publication (v4–v5) | **`DOLT_RESET --soft <pre-op commit>` + `DOLT_CHECKOUT` of the graph tables** when HEAD is still the operation commit; **`DOLT_REVERT <op commit>`** when HEAD has moved | A hard reset discards unrelated dirty work on a shared server; a per-table checkout reverts only the graph tables (verified against Dolt 2.1.8); a revert preserves later commits from other actors. |

### 2b. Proposed ruling amendments (pending operator ruling)

| # | Ruling | Earlier drafts said | v6 proposes | Evidence |
| --- | --- | --- | --- | --- |
| A1 | **9** ("the snapshot lease" in the obligation list) | dropped silently (v1); caller-supplied expectation (v2–v3) | **The per-call transaction is the v0 lease and asserts a store-owned authority witness:** the accessor loads the witness from the clone-local file and the body checks it *inside the same transaction* — Scope row identity, the hash-chained ledger head (exact-prefix identity), the lease row (a read selects it; a mutation `UPDATE`s it and requires one affected row, with the holder's installation key, epoch, an unexpired `expires_at` by database time, **and the `fence` cell value read in this transaction** in the predicate, and **sets `fence` to a fresh random value** — every lease write does, so a takeover, expiry reclaim, renewal, or concurrent mutation is a same-cell-different-value conflict Dolt reports as `1213`, replayed into a zero-row refusal; a predicate without the fence cell does **not** fence under Dolt's cell-wise merge, probed), and the graph-state version (the ordered per-table hashes of the eight replicated graph tables). Public request types carry **no** authority fields. The cursor type is opaque from P1. | Rounds 1, 3, 4, 5. |
| A2 | **7b + 12** (listener; "`bd serve` creates the Scope on first serve") | a sibling server (v1); `bd serve` mints (v2–v4) | **BDP rows mount inside `internal/httpapi`** as a conditional second route table behind the same `route()`. **Only `bd bdp serve` mints**, through a **staged startup** (the tree forbids a shared→exclusive gate upgrade): resolve the workspace, take the exclusive gates, mint through a temporary source, close it, then take the shared gates and serve. `bd bdp serve` inherits `errServeReadonly` and refuses without a held Scope; **`bd serve` converts every graph failure into "rows absent + notice"** and is byte-identical without a URL. **v0 serves BDP only from SQL-server workspaces** (the unit-of-work leg): embedded Dolt never serves, and a **registered backend's store arm** — which may itself be embedded — has no fence, so its rows are absent until the backend declares a `GraphPublication` capability (a later ruling). | Rounds 2–5. |
...
- **Replication/merge ADR as a P1 gate.** The merge entry points are not
  ...
  change**: the witness records the ordered `DOLT_HASHOF_TABLE()` hashes of
  the eight replicated graph tables (verified to exist in Dolt 2.1.8; the
...
graphops/                        PUBLIC LEAF (sibling of issueops/, memoryops/)
  ...
  ├─ capabilities.go             StateVersioner (per-provider graph-state version, in-tx);
  │                              GraphPublication (declared by a provider that can fence
  │                              hazard R itself — required for a registered backend to serve)
  ├─ writer.go                   (P3) Writer
...
internal/storage/authority/      THE WITNESS MANAGER (clone-local half; no SQL)
  ...
  Transition{Kind, Phase (begun|local_committed|published|config_written), PreHead,
             OpCommit, ExpectedRoots, RemotePreHead, ConfigIntent}
  LeaseClaim{InstallationKey, Epoch} — the workspace's claim (the fence cell is read in-tx)
  ...
  Load: plain read; a Pending record → recovery (resume publication if the remote ledger
    head is unchanged and the op commit is local; else undo) BEFORE any assertion — run by
    admin verbs and bd bdp serve only; bd serve treats Pending as rows-absent + notice
...
internal/storage/graphops/       TX-LEVEL SHARED BODY — all three legs call it
  ...
  assertAuthorityInTx(ctx, tx, w, claim, mutating): Scope row == w; ledger head; lease row —
    read: SELECT holder/epoch/expires_at/fence; mutation: UPDATE … SET heartbeat_at = ?,
    fence = <fresh random> WHERE id = 1 AND holder_installation_key = ? AND epoch = ?
    AND expires_at > NOW(6) AND fence = <value just read> with exactly one affected row
    (same-cell-different-value → 1213 → replay → 0 rows → refuse);
  ...
  ValidateStateInTx / graphStateVersionInTx (ordered DOLT_HASHOF_TABLE of eight tables)
...
- **`graphops.ScopeBootstrapper`** — `Mint`, once, under the exclusive gate
  through a temporary source (staged startup), fenced per hazard, multi-
  phase: precondition *no Scope row*; INSERT the singleton Scope row, seed
...
- **`graphops.Admin`** — `Promote` (precondition: Scope row CAS + the
  ...
  composition root under the exclusive gate; `httpapi.GraphConfig` has no
  field for it.
...
BeadGraphReader (manager-backed role): w := authority.Load(beadsDir); claim := process claim
   ...
   │ stmt 3  lease row: holder key/epoch == claim AND expires_at > NOW(6)   (SELECT — a read; a
   │         mutation UPDATEs it and rewrites the fence cell instead)
   │ stmt 4  graph-state version: ordered DOLT_HASHOF_TABLE() of the eight replicated graph
   │         tables == w.StateVersion → else ErrStateChanged (returned WITHOUT validating here;
