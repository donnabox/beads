...
| `SELECT … FOR UPDATE` on the Scope row to serialize the ledger (v4) | **A single-row sequence counter incremented with `UPDATE`** — the journal's `bd_events_seq` precedent | Dolt has no row locks; `FOR UPDATE` is a parse-only no-op (five in-tree comments). Contention on the counter row is a commit-time serialization loser that `withRetryTx`/`RunTxResult` replay. |
...
| A7 | **9** (promotion "explicit and epoch-rotating") — *new* | v5 partitioned by hazard but fenced a subset | **Fences compose by hazard; every replicated graph mutation is fenced inside its transaction and, on hazard R, published by one primitive.** *Hazard S — a shared database* (every SQL-server topology): the dolt-ignored **`graph_authority_lease`** row (the `leases`/`RunTxEphemeral` precedent) with the predicate in A1; renewal is ephemeral; expiry fails closed; promotion takes an expired lease (or `--steal`) and CASes the epoch. *Hazard R — a configured remote*: **every** replicated graph mutation (`Mint`, `Promote`, `Rotate`, `Install`, `LedgerApply`, the P1 seeds, P3 writes) runs through **`PublishGraphMutation`** on the provider: `DOLT_FETCH`; require the remote-tracking HEAD to be an **ancestor** of local HEAD (`DOLT_MERGE_BASE`); record the remote's graph roots and ledger head; the fenced transaction; a **scoped** commit (`DOLT_ADD` of the graph tables, `DOLT_COMMIT -m` — never `-Am`; a new `RunTxScopedResult` on the UOW leg, whose `Commit` hardcodes `-Am` today); `DOLT_PUSH` classified by a **typed** lift of the tree's untyped `pushRacePattern` (it already matches all three race routes) — on a race, refetch and compare **all** remote graph roots and the ledger head: any graph delta → fail closed and undo; unchanged graph roots (issue-plane divergence only) → keep the commit, `ErrSyncRequired`; any other failure → keep the commit, mark `unpublished`, retry later. Undo = soft reset + per-table checkout when HEAD is still the operation commit, `DOLT_REVERT` otherwise. **Reads on hazard R** require a remote observation no older than `bdp.authority_heartbeat` (a serving process fetches on a timer; a CLI read fetches when stale) and fail closed after the grace. The fence watcher is a state machine `held → renewing → lost`: on loss the BDP rows are disabled atomically (legacy surface untouched), `bd bdp serve` exits 3, and the watcher joins before provider shutdown. *Both hazards → both fences.* *Neither hazard* → not an authority-preserving topology (A5 residual); `--rotate-url` only. Force-push routes bypass the fence as operator acts. | Round 5 (Codex C2–C4, H5, H7, H10, H11; Gemini). |
...
internal/storage/authority/      THE WITNESS MANAGER (clone-local half; no SQL)
  ...
  LeaseClaim{InstallationKey, Nonce (per process), Epoch} — process-local, passed to bodies
  installation id: ~/.config/bd/installation-id (UserConfigYamlPath's directory; native
    fallback); created O_EXCL 0600, re-read, directory fsynced
  Load: plain read; a Pending record → recovery (resume publication if the remote is
    unchanged and the op commit is local; else undo) BEFORE any assertion
...
internal/storage/graphops/       TX-LEVEL SHARED BODY — all three legs call it
  ...
  assertAuthorityInTx(ctx, tx, w, claim, mutating): Scope row == w; ledger head; lease row —
    read: SELECT holder/epoch/expires_at; mutation: UPDATE … SET heartbeat_at = ? WHERE id = 1
    AND holder_installation_key = ? AND holder_nonce = ? AND epoch = ? AND expires_at > NOW(6)
    with exactly one affected row; graph-state version == w.StateVersion else ErrStateChanged
  nextLedgerSeqInTx: UPDATE graph_ledger_seq SET next = next + 1 WHERE id = 0
...
internal/storage/uow/beadgraph_*.go        THE SERVING LEG
  ...
  newSQLServerUOWProvider today, dropped) and timedProvider forwards it; NEW
...
BeadGraphReader (manager-backed role): w := authority.Load(beadsDir); claim := process claim
   ...
   │ stmt 3  lease row: holder key/nonce/epoch == claim AND expires_at > NOW(6)   (SELECT — a read)
