...
- **Replication/merge ADR as a P1 gate — RULED 2026-09-07 (plan ruling 14, option B: the gate plus a four-law charter).** The merge entry points are not
  ...
  under the storage spec B4 preimage
...
internal/storage/authority/      THE WITNESS MANAGER (clone-local half; no SQL)
  ...
  Load: diagnostic read only; it does not recover, grant authority, or clear Unverified.
    Pending/unverified state refuses positive authority assertions. Recovery by fresh provider
    evidence under the held guard is an explicit administrative operation, never a side effect
    of Load or an ordinary protected read; see the storage spec's B4 witness clarification.
    Recovery runners remain admin verbs and bd --graph-mode link serve startup, per the
    identity matrix; bd serve and CLI reads report rows-absent / ErrNotAuthority with a notice.
...
internal/storage/graphops/       TX-LEVEL SHARED BODY — all three legs call it
  ...
  assertAuthorityInTx(ctx, tx, w, claim, mutating): Scope row == w; ledger head; lease row —
    read: SELECT the lease row (all B4 columns), NOW(6), and @@session.time_zone;
    require expires_at > NOW(6) (reads never
  ...
  ValidateStateInTx / graphStateVersionInTx: eight fixed DOLT_HASHOF_TABLE expressions in
    one SELECT, in B4 order; validate each 32-character lowercase base32 label, concatenate
    its ASCII bytes without separator/tag (256 bytes total), SHA-256, then lowercase hex.
    DATABASE()/ACTIVE_BRANCH() are separate observed binding facts, not digest inputs.
    Leg connection setup pins time_zone='+00:00' before graph SQL, outside the body and budget;
    the lease observation records
    @@session.time_zone and protected assertions refuse another zone. Protected reads and
    mutations use the provider-bound workspace database on its configured default branch.
...
- Replication mechanisms and qualification gates are recorded in [the replication ADR](BDP_GRAPH_REPLICATION_ADR.md). Its shared-database exclusion and recovery proofs remain open; the document alone does not clear the P1 migration gate.
